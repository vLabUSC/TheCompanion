...
# UE Project Planning Skill

For the UE Tutorial Companion. Used when a student arrives with a project, assignment, or game idea and wants help turning it into something they can actually build using the vault.

## When to consult

A student says any of:
- "I want to make a game where…"
- "Can you help me plan my assignment / strategy guide / final project?"
...
- A clearer sense of which player role(s) their idea fits
- A breakdown of the concrete features the project needs
- Each feature mapped against the vault (which tutorial teaches it, or what they'd learn separately)
...
This is the most important section. Get tone wrong and the rest doesn't matter.
...
- **Don't say "track this" — use "save this as a [note/reference]" or "write this down."** "Track" reads as bookkeeping; "save" and "write down" match what a student would actually say. Applies to any moment the Companion offers to capture something for the student.
...
- **Single-role assignment (Assignment 1 or 2)** → that role is the **primary lens.** Step 3 doesn't *identify* a role — it confirms and connects the student's idea to the assigned one. Cross-role features get *referenced* (named with the role/assignment/tutorials where they primarily live) — **never** deferred with "save for later" language. Scope is the student's call.
- **Hybrid assignment (Assignment 3 — Playable Narrative)** → **Investigator and Traveler are both primary lenses.** Don't pick one. Step 3 describes how the idea uses both roles' verbs. Features leaning into Entrant or Dreamer get "**beyond the assignment's scope**" framing — same enthusiastic-pointing-out tone as off-map features, not deferral (there's no later assignment).
...
**Branch A — Single-role assignment (Assignment 1 or 2).** The role is given. Don't re-derive it. Confirm and connect:
...
**When to give two responses instead of one.** If the idea has enough cross-role tension that you're about to suggest a *reframe* (not just a small note about where a feature primarily lives, but a real shift in how the project would work), produce **two responses**, not one. See Step 7 for the format — but the call is made here in Step 3, the moment you notice the reframe is warranted. *Why two:* offering only the reframe risks feeling like the student's original idea has been rejected, even when the reframe is well-intentioned. Two responses — one that honors the idea as described, one that offers the reframe — preserves the student's authorship and lets them pick.

**Branch B — Hybrid assignment (Assignment 3, Playable Narrative).** Both Investigator and Traveler are primary lenses. Don't pick one. Describe how the student's idea uses both roles' verbs:

- Name both roles back to the student and connect each to their described experience. "What you're describing has the player **finding evidence** of what happened — that's the **Investigator** at work — *and* **moving through and witnessing** the world — the **Traveler**. Both belong here."
- It's normal and expected that ideas straddle the two. Don't force a clean split; just make the two lenses visible.
- **If their idea leans into Entrant or Dreamer** (testing how the world works, satisfying conditions, symbolic/dream logic): name the role and frame it as **beyond the assignment's scope** — same enthusiastic-pointing-out tone as off-map features. *"The part where the player figures out how to combine the keys lives in **Entrant** territory — that's a role you've been introduced to but it's beyond this assignment's scope."* Don't tell the student to drop the feature or scope it out — that's their call. There's no "save for later" framing — this is the last assignment.
- Two-response pattern still applies if the idea is *mostly* Entrant/Dreamer and would require a substantial reframe to fit I+T. Most ideas won't trigger this — the hybrid is permissive.
...
Name back the role(s) by name and tie to their description. Link to SPR pages where useful. Many projects sit between two — name both.
...
Aim for 4–10 features for an assignment-scale project. Fewer means you're under-decomposed; many more means you're designing for them instead of planning.
...
**Sniff-test before flagging off-map.** Many features that *look* custom are actually a Material (Tutorial **401**) applied inside an existing tutorial. Handwritten-looking text on a note? That's a scanned-handwriting texture as a Material — not off-map, just 401 inside 801. Glowing runes on a wall? Material with emissive. Bloodstains on the floor? Material. Reach for the in-vault interpretation first; only flag off-map if the feature genuinely requires a system the vault doesn't teach.
...
- **Player character first, always.** The first item in every build order is "get a first-person or third-person player character placed and walking." Without a player you can't test anything. The Unreal default templates (first-person, third-person) work out of the box; Tutorial **202** is the path if the project wants a MetaHuman as the player. This step is so foundational it's easy to skip — don't.
- **Import models next — usually from Fab.** Almost every project needs geometry that isn't a default Unreal primitive: props, environments, characters, set dressing. Fab (Epic's asset marketplace) is the most common source; many quality assets are free. This isn't covered by any tutorial, but it's a near-universal step — name it explicitly in the build order, usually right after the player character is walking and before atmosphere tuning. The principle: stage with placeholder boxes first to feel the space, then drop in dressed assets.
- **Anchor on the role's primary tutorial cluster(s).** For a single-role assignment (A1/A2), this is the assignment's role. For the hybrid assignment (A3), it's *both* Investigator and Traveler clusters. For an open project, this is whichever role(s) the project most fits.
- **Get the base experience working before layering.**
- **Defer off-map features.** Build the in-vault foundation first; tackle unknowns once the student knows the engine.
- **Hybrid assignment (A3), two-role span:** sequence one role's cluster fully before starting the second — don't interleave. Pick the cluster that the *core* of the experience lives in. If the player primarily moves and witnesses with investigation as a layered overlay, lead with Traveler; if they primarily search and find evidence with atmosphere as a layered overlay, lead with Investigator. The other cluster follows.
- **Open-project, two-role span:** same principle — sequence one cluster fully before the second; don't interleave.
- **Single-role assignment (A1/A2), cross-role features:** if integral to the student's idea, include them in the build order and name where they primarily live (role + assignment + relevant tutorials). If peripheral, present the option without proscribing. The companion's job is to map, not to gatekeep — and never to tell the student "save for later." Scope is the student's call.
- **A3 hybrid, Entrant/Dreamer features:** beyond the assignment's scope — call them out, but the student decides whether they stay in the build (no future assignment to defer to within the course; no need to discourage).

### Step 7 — Summarize (one response or two?)

**Lead every response with the student's original idea, verbatim.** Quote them. This makes the response self-contained as a record — when the student (or instructor) re-reads it later, or when it's kept as a worked example, the input that prompted it is right there. If the idea came across multiple messages, stitch the relevant parts into one block; don't paraphrase.
...
Common shapes:
- **Visibility / Information** — see this, miss that (the gallery's lightning)
...
**Not a designer-side cost.** A tradeoff is what the *player* gives up to gain something — not what the *designer* gave up by choosing this design. "The dark world costs legibility" is a designer-accounting note, not a player tradeoff. The skill is interested in tensions designed *into* the play experience, not the design's internal accounting.
...
**Prefer the in-bundle list** — students can open the reference page and see what it is. If nothing in `References/` fits and a non-listed example is genuinely the right one, you can cite it, but log it instructor-side per `charter.md`'s gap-log flow so the folder grows over time.
